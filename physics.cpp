...
void DoWallCollisions(double newx, double newy)
{
	...
	while(repeat)
	{
		...
		for(i=0;i<sector->numwalls;i++)
		{
			...
			if(wall->flags==WALL_NORMAL)  if(s<-wall->soffset || s>1.0+wall->soffset) continue;
			
			if(wall->flags==WALL_ADJOINED) if(s<wall->soffset || s>1.0-wall->soffset) continue;
			...
			if(dpwall-dper<2.0)
			{
				*(walllist+i)=1;
				...
				{
					if((sectors+wall->adjoin)->floorheight-(player.height-player.fromfloor)<1)
						if((sectors+wall->adjoin)->ceilingheight-(sectors+wall->adjoin)->floorheight>player.fromfloor)
					{
						if(dpwall-dper>0) continue;
											
						player.sector=wall->adjoin;
						newsector=wall->adjoin;
						player.height=(sectors+wall->adjoin)->floorheight+player.fromfloor;
						repeat=1;
						break;
