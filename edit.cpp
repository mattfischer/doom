	...
	for(i=0;i<numsectors;i++)
	{
		for(j=0;j<sectors[i].numwalls;j++)
		{
			...
			if(abs(x0-x)<3 && abs(y0-y)<3)
			{
				...
				{
					point3=&(sectors[sectors[i].walls[j].adjoin].walls[sectors[i].walls[j].mirror].end);
					point4=&(sectors[sectors[i].walls[j].adjoin].walls[(sectors[i].walls[j].mirror+sectors[sectors[i].walls[j].adjoin].numwalls-1)%sectors[sectors[i].walls[j].adjoin].numwalls].start);
				}
			}
